...
### 菜单注入依赖官方 bundle 的锚点结构

「不使用工作区」菜单项通过匹配官方 `dsh-client-ui-workspace` 构建产物中的三个锚点字符串完成注入。这是「组合优先」原则下最接近边界的部分：锚点是官方内部实现，官方改版（变量改名、压缩、组件重构）会使锚点失配。

- **当前行为**：启动时探测一次并输出日志——全部匹配打 info（`picker menu injection ready`），失配打 warn 并列出缺失锚点；serve 时同样失配即原样返回 bundle（页面不受影响）。菜单项静默降级为「可见预设 + `/readonly-session` 命令」两个入口，功能不丢失，只是少了快捷方式。
- **为什么可接受**：官方 bundle 未压缩、锚点自项目早期至今稳定；注入的是通用注册表，官方默认行为不变；零磁盘改动，升级无需重放。
- **迁移触发条件**：官方（或社区）提供工作区选择器的正式菜单贡献扩展点时，应迁移到该扩展点并移除注入路由。迁移前每次官方升级后留意启动日志中的 warn。

### 无其他已知风险

锁定、审批、隔离目录全部基于官方公开 API 与日志事件，官方语义由上游测试约束；其余设计取舍（权限旋钮不锁定等）已在安全模型一节如实披露。
